...
export default function App() {
  ...
  useEffect(() => {
    ...
    const roomSub = supabase.channel(`room-${roomCode}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'rooms', filter: `code=eq.${roomCode}` }, (payload) => {
        ...
        setScreen(data.game_state); setGameWord(data.game_word); setSettings(data.settings); setTurnInfo(data.turn_info);
      }).subscribe();
  ...
  return (
    <div className="screen">
      ...
      <p style={{ textAlign: 'center', fontSize: '0.6rem', color: '#aaa', marginTop: '-15px', marginBottom: '10px' }}>v1.5.3 - FIX Flujo Manual</p>
      ...
      <AnimatePresence mode="wait">
        ...
        {(screen !== 'START' && screen !== 'LOBBY') && (
          <GamePhase screen={screen} players={players} playerId={playerId} gameWord={gameWord} settings={settings} roomCode={roomCode} isHost={isHost} turnInfo={turnInfo} />
        )}
      </AnimatePresence>
    </div>
  );
...
function GamePhase({ screen, players, playerId, gameWord, settings, roomCode, isHost, turnInfo }: any) {
  ...
  const handleNextStage = async (nextScreen: Screen) => {
    await supabase.from('rooms').update({ game_state: nextScreen }).eq('code', roomCode);
  ...
  if (screen === 'WRITING') {
    return (
      <div className="card">
        <h3 style={{display:'flex', alignItems:'center', gap:'10px'}}><PenTool size={20}/> Tu Pista</h3>
        <input className="input" value={clueInput} onChange={(e) => setClueInput(e.target.value)} placeholder="Ej: Es redondo..." disabled={!!me.clue} />
        <button className="btn btn-primary" onClick={async () => await supabase.from('players').update({ clue: clueInput }).eq('id', playerId)} disabled={!!me.clue || !clueInput}>
          {me.clue ? 'Enviado ✅' : 'ENVIAR'}
        </button>
        {isHost && <button className="btn btn-accent" style={{marginTop:'20px'}} onClick={() => handleNextStage('DEBATE')}>IR AL DEBATE</button>}
      </div>
    );
  }
  ...
  if (screen === 'DEBATE') {
    return (
      <div className="card">
        <h3 style={{display:'flex', alignItems:'center', gap:'10px'}}><MessageSquare size={20}/> Debate</h3>
        ...
        {isHost && <button className="btn btn-accent" style={{marginTop:'20px'}} onClick={() => handleNextStage('VOTING')}>IR A LA VOTACIÓN</button>}
      </div>
    );
  }
  ...
  if (screen === 'VOTING') {
    return (
      <div className="card">
        <h3 style={{display:'flex', alignItems:'center', gap:'10px'}}><CheckCircle2 size={20}/> Votación</h3>
        <div className="grid">
          {players.map((p: any) => (
            <button key={p.id} className={`btn ${votedPlayerId === p.id ? 'btn-primary' : 'btn-secondary'}`} onClick={() => isHost && setVotedPlayerId(p.id)}> {p.name} </button>
          ))}
        </div>
        {isHost && votedPlayerId && (
          <button className="btn btn-accent" style={{marginTop:'20px'}} onClick={() => handleNextStage('RESULT')}> REVELAR SI ES EL IMPOSTOR </button>
        )}
      </div>
    );
  }
