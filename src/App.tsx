      ...
      <AnimatePresence mode="wait">
        {screen === 'START' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="card">
            ...
            <button className="btn btn-primary" onClick={() => { if (!currentPlayerName.trim()) return alert('Poné tu nombre'); setMode('LOCAL'); setPlayers([{id:'1', name:currentPlayerName}]); setCurrentPlayerName(''); setScreen('LOBBY'); setIsHost(true); }}>
        ...
        {screen === 'LOBBY' && (
          <div className="card">
            ...
            <label>Jugadores conectados ({players.length}):</label>
            <div className="grid">
              {players.map(p => <div key={p.id} className="btn btn-secondary" style={{ fontSize: '0.8rem' }}>{p.name}</div>)}
            </div>
            {isHost && (
              <>
