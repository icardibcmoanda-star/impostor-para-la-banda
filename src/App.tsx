...
export default function App() {
  ...
  // Lógica de auto-avance (Solo Host Online)
  useEffect(() => {
      ...
      if (allReady) {
        setTimeout(() => {
          supabase.from('rooms').update({ 
            game_state: settings.writtenClues ? 'WRITING' : 'DEBATE'
          }).eq('code', roomCode);
  ...
  const startLocalGame = () => {
    ...
    setGameWord(word);
    setPlayers(players.map(p => ({ ...p, is_impostor: imps.includes(p.id), is_ready: false })));
  ...
  return (
    <div className="screen">
      ...
      <p style={{ textAlign: 'center', fontSize: '0.6rem', color: '#aaa', marginTop: '-15px', marginBottom: '10px' }}>v1.5.0 - Versión Final con Todo</p>
...
function GamePhase({ screen, setScreen, mode, players, playerId, revealedIdx, setRevealedIdx, gameWord, settings, roomCode, isHost, turnInfo, chatMessages, setChatMessages }: any) {
  ...
  const handleNext = async () => {
    if (mode === 'LOCAL') {
      ...
      else setScreen(settings.writtenClues ? 'WRITING' : 'DEBATE');
    } else {
  ...
  if (screen === 'REVEAL') {
    return (
      <div className="card">
        ...
        {show && (
          <button className={`btn ${me?.is_ready ? 'btn-secondary' : 'btn-primary'}`} style={{ marginTop: '20px' }} onClick={handleNext} disabled={me?.is_ready}>
            {me?.is_ready ? `ESPERANDO (${players.filter((p:any)=>p.is_ready).length}/${players.length})` : 'LISTO'}
          </button>
        )}
      </div>
    );
    ...
    return (
      <div className="card" style={{ height: '75vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', background: '#f0f0f0', padding: '10px', borderRadius: '10px', marginBottom: '10px' }}>
          {chatMessages.map((m: any, i: number) => ( <div key={i} style={{ marginBottom: '5px', background: 'white', padding: '8px', borderRadius: '8px' }}><strong>{m.sender}:</strong> {m.text}</div> ))}
        </div>
        <div style={{ display: 'flex', gap: '5px' }}>
          <input className="input" value={chatInput} onChange={(e) => setChatInput(e.target.value)} style={{marginBottom: 0}} />
          <button className="btn btn-primary" onClick={() => { if (mode === 'LOCAL') setChatMessages([...chatMessages, {sender: me.name, text: chatInput}]); else supabase.from('rooms').update({ chat_messages: [...chatMessages, {sender: me.name, text: chatInput}] }).eq('code', roomCode); setChatInput(''); }} style={{width: 'auto', marginBottom: 0}}><Send size={18}/></button>
        </div>
        {isHost && <button className="btn btn-accent" style={{ marginTop: '10px' }} onClick={async () => { if (mode === 'LOCAL') setScreen('VOTING'); else await supabase.from('rooms').update({ game_state: 'VOTING' }).eq('code', roomCode); }}>VOTAR</button>}
      </div>
    );
  }
  ...
  if (screen === 'VOTING') {
    return (
      <div className="card">
        ...
        <div className="grid">
          {players.map((p: any) => <button key={p.id} className="btn btn-secondary" onClick={async () => { if (mode === 'LOCAL') setScreen('RESULT'); else if (isHost) await supabase.from('rooms').update({ game_state: 'RESULT' }).eq('code', roomCode); }}>{p.name}</button>)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="card" style={{ textAlign: 'center' }}>
      ...
      <p>Infiltrados: <strong>{players.filter((p:any) => p.is_impostor).map((p:any) => p.name).join(', ')}</strong></p>
      <button className="btn btn-primary" onClick={async () => { if (mode === 'LOCAL') { setScreen('LOBBY'); } else if (isHost) await supabase.from('rooms').update({ game_state: 'LOBBY' }).eq('code', roomCode); }}>OTRA PARTIDA</button>
    </div>
  );
}
