...
export default function App() {
  ...
  const [settings, setSettings] = useState<GameSettings>({
    impostorsKnowEachOther: true, revealSubCategory: true, giveImpostorClue: true, useTimer: true, writtenClues: true
  });
  ...
  const startLocalGame = () => {
    if (players.length < 3) return alert('Mínimo 3 jugadores');
    if (selectedCategory.isCustom) { setScreen('INPUT_CUSTOM'); return; }
    const word = getRandomWord(selectedCategory.items, selectedCategory.id);
    const shuffled = [...players].sort(() => Math.random() - 0.5);
    const imps = shuffled.slice(0, impostorCount).map(p => p.id);
    const starter = players[Math.floor(Math.random() * players.length)].name;
  ...
  return (
    <div className="screen">
      ...
      <p style={{ textAlign: 'center', fontSize: '0.6rem', color: '#aaa', marginTop: '-15px', marginBottom: '10px' }}>v1.7.3 - Fix Pantalla Blanca</p>
...
function GamePhase({ screen, setScreen, mode, players, setPlayers, playerId, revealedIdx, setRevealedIdx, gameWord, setGameWord, settings, roomCode, isHost, turnInfo, setTurnInfo, getRandomWord, impostorCount, selectedCategory }: any) {
  ...
  if (screen === 'INPUT_CUSTOM') {
    const handleCustomSubmit = async () => {
      ...
      if (mode === 'LOCAL') {
        ...
        if (revealedIdx < players.length - 1) { setTurnInfo({...turnInfo, customWords: allWords}); setRevealedIdx(revealedIdx + 1); setCustomNames(['','','']); }
          ...
          setTurnInfo({...turnInfo, customWords: allWords}); setRevealedIdx(0); setScreen('REVEAL');
        }
      } else {
    ...
    return (
      <div className="card">
        <h3>👥 Conocidos</h3>
        <p style={{fontSize:'0.8rem'}}>Escribí 3 nombres de gente que todos conozcan.</p>
  ...
  if (screen === 'WRITING') {
    return (
      <div className="card">
        ...
        <input className="input" value={clueInput} onChange={(e) => setClueInput(e.target.value)} placeholder="Tu pista..." disabled={!!me?.clue} />
        <button className="btn btn-primary" onClick={async () => { if (mode === 'LOCAL') setScreen('DEBATE'); else await supabase.from('players').update({ clue: clueInput }).eq('id', playerId); }}>ENVIAR</button>
        {isHost && <button className="btn btn-accent" style={{marginTop:'10px'}} onClick={() => handleNextStage('DEBATE')}>IR AL DEBATE</button>}
      </div>
    );
  }
  ...
  if (screen === 'VOTE_RESULT') {
    ...
    const nextRound = async () => {
      ...
      if (remainingImps > 0 && players.length - newEliminated.length > remainingImps + 1) {
        const word = getRandomWord(selectedCategory.items, selectedCategory.id);
        if (mode === 'LOCAL') { setGameWord(word); setTurnInfo({...turnInfo, eliminated: newEliminated, votes: {}, round: (turnInfo.round || 1) + 1}); setRevealedIdx(0); setScreen('REVEAL'); }
        else { await supabase.from('rooms').update({ game_state: 'REVEAL', game_word: word, turn_info: {...turnInfo, eliminated: newEliminated, votes: {}, round: (turnInfo.round || 1) + 1, chat: []} }).eq('code', roomCode); }
      } else { handleNextStage('RESULT'); }
    };
  ...
  return (
    <div className="card" style={{ textAlign: 'center' }}>
      ...
      {isHost && <button className="btn btn-primary" onClick={async () => { if (mode === 'LOCAL') setScreen('LOBBY'); else await supabase.from('rooms').update({ game_state: 'LOBBY', turn_info: { starter: null, direction: 'Horario', chat: [], customWords: [], votes: {}, eliminated: [], round: 1 } }).eq('code', roomCode); }}>OTRA PARTIDA</button>}
    </div>
  );
}
